...
def revert_transcript_prompt(apps, schema_editor):
    TranscriptConfig = apps.get_model('transcripts', 'TranscriptConfig')
    TranscriptConfig.objects.filter(type='TRANSCRIPT_PROMPT').update(value=TRANSCRIPT_PROMPT_V1)


class Migration(migrations.Migration):

    dependencies = [
        ('transcripts', '0005_transcript_coverage_warning'),
    ]
