...
const qIndex    = ref(0)
const result    = ref(null)     // submit response
...
async function load() {
  ...
  error.value = null
  try {
    const res = await videosService.getLessonExam(props.courseSlug, props.lessonSlug)
    exam.value = res.data
  } catch (e) {
...
async function submitQuiz() {
  ...
  try {
    ...
    // Refresh exam to get updated user_progress
    const refreshed = await videosService.getLessonExam(props.courseSlug, props.lessonSlug)
...
function getQuestionById(id) {
  return exam.value?.questions?.find(q => q.public_id === id)
}
  ...
  <div class="quiz">
    ...
    <div v-else-if="state === 'idle'" class="quiz__idle">
      ...
      <div v-if="exam.user_progress" class="quiz__last-result">
        <span>
          Lần trước: <strong>{{ exam.user_progress.score }}/100</strong>
    ...
    <div v-else-if="state === 'in_progress' && currentQ" class="quiz__progress">
      ...
      <div class="quiz__question-card">
        ...
        <!-- Yes/No or True/False (two wide buttons) -->
    ...
    <div v-else-if="state === 'submitted' && result" class="quiz__result">
      ...
      <details class="quiz__review">
        ...
        <div class="quiz__review-list">
          <div
            v-for="(shuffledQ, i) in shuffled"
            :key="shuffledQ.public_id"
            class="quiz__review-item"
            :class="isCorrect(shuffledQ.public_id, answers[shuffledQ.public_id]) ? 'quiz__review-item--correct' : 'quiz__review-item--wrong'"
          >
            <span class="quiz__review-icon">
              {{ isCorrect(shuffledQ.public_id, answers[shuffledQ.public_id]) ? '✅' : '❌' }}
            </span>
            <div>
              <p class="quiz__review-q">Câu {{ i + 1 }}: {{ shuffledQ.question_text }}</p>
              <p class="quiz__review-ans">
                Đúng: <strong>{{ getCorrectText(shuffledQ) }}</strong>
                <span v-if="!isCorrect(shuffledQ.public_id, answers[shuffledQ.public_id])">
                  · Bạn chọn: {{ getOptionText(shuffledQ, answers[shuffledQ.public_id]) || '(chưa trả lời)' }}
                </span>
              </p>
            </div>
          </div>
...
.quiz__option--selected .quiz__option-id {
  background: var(--accent-gold);
  color: #2E1A0F;
}
