...
const currentCard = computed(() => pool.value.cards[index.value] ?? null)
const progress    = computed(() => `${index.value + 1} / ${pool.value.count}`)
...
async function loadCards() {
  ...
  error.value = null
  try {
    const res = await videosService.getLessonFlashcards(props.courseSlug, props.lessonSlug)
    pool.value = res.data
...
function next() {
  isFlipped.value = false
  if (index.value < pool.value.count - 1) {
    index.value++
  } else {
...
<template>
  <div class="fc">
    ...
    <!-- Empty -->
    <div v-else-if="pool.total_in_pool === 0" class="fc__empty">
    ...
    <div v-else-if="sessionDone" class="fc__done">
      ...
      <p class="fc__done-sub">{{ pool.count }} thẻ đã xem</p>
    ...
    <template v-else-if="currentCard">
      ...
      <div class="fc__header">
        <div class="fc__pool-info">
          🃏 Kho {{ pool.total_in_pool }} thẻ
          <span v-if="pool.due_count > 0" class="fc__due-badge">{{ pool.due_count }} đến hạn</span>
        </div>
    ...
    <div v-if="confirmReshuffle" class="fc__overlay" @click.self="confirmReshuffle = false">
      <div class="fc__dialog">
        <p class="fc__dialog-msg">Bạn đang ở thẻ {{ index + 1 }}/{{ pool.count }}.<br>Xáo bài sẽ đặt lại phiên học hiện tại.</p>
